...
button.addEventListener('click', function () {
  ...
  fetch('https://api.openweathermap.org/data/2.5/forecast?zip=' + inputStr + '&appid=1e12d953ad0dc8ff3a4f488ae456a976')
    .then((res) => res.json())
    .then((data) => {
      ...
      var feelsLikeTemp = (feelsLike.innerHTML = Math.round(feels_Like - 273.15));
      ...
      var d1 = 'http://openweathermap.org/img/wn/' + data.list[4]['weather'][0].icon + '@2x.png';
      dayOneIcon.innerHTML = '<img src=' + d1 + '>';

      var d2 = 'http://openweathermap.org/img/wn/' + data.list[12]['weather'][0].icon + '@2x.png';
      dayTwoIcon.innerHTML = '<img src=' + d2 + '>';

      var d3 = 'http://openweathermap.org/img/wn/' + data.list[12]['weather'][0].icon + '@2x.png';
      dayThreeIcon.innerHTML = '<img src=' + d3 + '>';

      tempConvert(primeryTemp, feelsLikeTemp, day1temp, day2temp, day3temp, windspeed);
      weatherIcon(iconCode);
      weatherDate(stringArray);

      input.value = '';
    })
    .catch(() => alert('Invalid City Zip Code. Please try again'));
});
...
function tempConvert(maintemp, feelsLikeT, temp1, temp2, temp3, windspeed) {
  ...
  wind.innerHTML = Math.round((windspeed * 3.6 * 10) / 10) + 'kph';
  ...
  btnFar.addEventListener('click', function () {
    ...
    wind.innerHTML = Math.round((windspeed * 2.237 * 10) / 10) + 'mph';
  });
}
